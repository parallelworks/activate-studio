// The coordination plane: task-scoped tokens over MCP, and a claim
...
const { createTask, stopTask, getTask } = await import('../dist/tasks.js')

const app = Fastify()
await app.register(mcpRoutes)

const m = createTask({ objective: 'board test', model: 'stub', agents: [{ objective: 'a' }, { objective: 'b' }], maxAgents: 4 })
const call = (token, agent, name, args = {}) => app.inject({
  method: 'POST', url: '/api/mcp',
  headers: { authorization: `Bearer ${token}`, 'x-task-agent': agent },
...
test('board tools appear only for a task token', async () => {
...
test('status and posts reach the board and the subtask tree', async () => {
