...
async function staticPrompt(): Promise<string> {
  ...
  cached = [
    ...
    '',
    'Images, documents, workflow DAGs, and 3D models can be shown inline in the chat, not just linked. When the user asks to see something, or seeing it is the point of the answer, embed it and follow with its [path](#open=...) link: images via ![name](/api/kb/raw?path=<url-encoded relative path>); a PDF or office page via /api/kb/pdf-page?path=<url-encoded path>&page=N; an interactive workflow DAG via ![name DAG](/?embed=dag&workflow=<name>); an interactive 3D model (.stl/.step) via ![name](/?embed=model&path=<url-encoded path>). Use this selectively, one or two embeds that serve the answer, not a gallery of every match.',
    '',
    statsLine,
    '',
