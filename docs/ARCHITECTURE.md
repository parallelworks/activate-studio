...
Our index lives at `index/gufi/PW_KNOWLEDGE_BASE/` inside the repo. The build (`indexer/reindex.sh`) excludes `.git`, `node_modules`, `.venv`, `__pycache__`, `dist`, `build`, caches, screenshots, and dot-directories via a `--skip-file`. The real corpus is roughly 3,400 files across 320 directories, dominated by markdown, DOCX, PDF, and Python.
...
The system prompt is composed at startup: an instruction to ground answers in the KB and cite paths, corpus statistics, the account's workflow list, and the entire knowledge base `CLAUDE.md`, whose accuracy guardrails (claims not to make, IL terminology, no user counts) bind chat output because chat output over this corpus can end up in deliverables. Today's date is injected per request so deadline questions distinguish past from upcoming.
...
- Known failure modes: lembed segfaults on token-dense chunks are handled by the 800/400 caps; if a directory still fails it is logged and remains covered by full-text search. Endpoint tunnels can wedge while the process lives (platform note from the nii-ops-viewer README); restart the endpoint process.
